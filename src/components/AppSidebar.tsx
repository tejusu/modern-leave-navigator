...
export function AppSidebar() {
  ...
  return (
    <Sidebar collapsible="icon" className="border-r-0 bg-white dark:bg-gray-50">
      <SidebarHeader className="p-4 border-b border-gray-100 dark:border-gray-200">
        <div className="flex items-center justify-between group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:gap-2">
          <Button
            variant="ghost"
            className="size-10 p-0 order-last group-data-[collapsible=icon]:order-none hover:bg-gray-50 dark:hover:bg-gray-100 rounded-full transition-all duration-200"
            onClick={toggleSidebar}
          >
            <ChevronsLeft className="size-5 shrink-0 transition-transform duration-300 group-data-[collapsible=icon]:rotate-180 text-gray-600 dark:text-gray-700" />
          </Button>
        ...
        <SidebarMenu className="space-y-2">
          {menuItems.map((item) => (
            <SidebarMenuItem key={item.label}>
              <SidebarMenuButton 
                asChild 
                className="w-full justify-start px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-red-50 dark:hover:bg-red-50 hover:shadow-sm group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:rounded-full group-data-[collapsible=icon]:size-12 text-gray-800 dark:text-gray-800 hover:text-red-600 dark:hover:text-red-600"
              >
                <a href={item.href} className="flex items-center gap-3 group-data-[collapsible=icon]:justify-center">
                  <item.icon className="size-5 shrink-0 text-red-600 dark:text-red-600" />
      ...
      <SidebarSeparator className="bg-gray-200 dark:bg-gray-300" />
          ...
          <SidebarMenuItem>
            ...
            >
              <a href={settingsItem.href} className="flex items-center gap-3 group-data-[collapsible=icon]:justify-center">
                <settingsItem.icon className="size-5 shrink-0 text-red-600 dark:text-red-600" />
          ...
          <SidebarMenuItem>
            ...
            >
              <a href={logoutItem.href} className="flex items-center gap-3 group-data-[collapsible=icon]:justify-center">
                <logoutItem.icon className="size-5 shrink-0 text-red-600 dark:text-red-600" />
